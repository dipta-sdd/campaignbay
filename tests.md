...
- [ ] **TC-2.1:** **Admin Menu:** After activation, verify the "Campaigns" menu item appears correctly in the WordPress admin sidebar with the correct SVG icon.
    - **Expected:** Menu item is present. Icon is correctly sized and aligned.
- [ ] **TC-2.2:** **Submenu Navigation:** Click on the main "Campaigns" menu and all its submenus (Dashboard, All Campaigns, Add Campaign, Settings, Help).
    - **Expected:** All pages load correctly without errors. The React application initializes properly on each page.
- [ ] **TC-2.3:** **"Settings" Link on Plugins Page:** Navigate to the main WordPress Plugins page.
...
- [ ] **TC-2.4:** **Logs Modal:** Navigate to Settings > Global. Click the "View Logs" button.
...
- [ ] **TC-2.5:** **Clear Logs Functionality:** In the logs modal, click "Clear Log Files" and confirm the action.
...
- [ ] **TC-3.1.1:** **Create a Future-Dated Campaign:**
...
- [ ] **TC-3.1.2:** **Create an Immediately Active Campaign:**
    1.  Go to "Add Campaign".
    2.  **Discount Type:** `Scheduled Discount`.
    3.  **Status:** `Active`.
...
- [ ] **TC-3.1.3:** **Create a Draft Campaign:** Create any scheduled discount but set the **Status** to `Inactive`.
...
- [ ] **TC-3.2.1:** **Shop Page Display:** With the "Immediately Active" campaign (TC-3.1.2) running, view the main shop page.
    - **Expected:** All products show a strikethrough regular price and the new, 15% lower price. The "Sale!" badge appears on products.
- [ ] **TC-3.2.2:** **Single Product Page Display:** View the product page for a simple product.
...
- [ ] **TC-3.3.1:** **Basic Cart Application:** Add a product to the cart with the "Immediately Active" campaign running.
...
- [ ] **TC-3.3.2:** **Setting: Exclude Sale Items (ON):** In settings, turn ON "Automatically Exclude Sale Items". Add a product that has a native WooCommerce sale price to the cart.
    - **Expected:** The product's price in the cart is the native WooCommerce sale price. The CampaignBay discount is **not** applied.
- [ ] **TC-3.3.3:** **Setting: Exclude Sale Items (OFF):** In settings, turn OFF "Automatically Exclude Sale Items". Add the same native sale product to the cart.
...
- [ ] **TC-3.3.4:** **Setting: Stacking (Disabled):** In settings, turn OFF "Allow Stacking with WooCommerce Coupons". With a campaign discount active in the cart, try to apply a valid WooCommerce coupon.
    - **Expected:** The coupon is rejected. A clear error message is displayed ("A store promotion is already active...").
- [ ] **TC-3.3.5:** **Setting: Stacking (Enabled):** In settings, turn ON "Allow Stacking...". Repeat the previous test.
...
- [ ] **TC-3.4.1:** **Scheduled Activation:** Let the start time for your future-dated campaign (TC-3.1.1) pass. Visit the site to trigger WP-Cron.
...
- [ ] **TC-3.4.2:** **Scheduled Deactivation:** Let the end time for the same campaign pass. Visit the site.
...
- [ ] **TC-3.4.3:** **Manual Deactivation:** While a scheduled campaign is running, edit it and change its status to `Draft`.
...
- [ ] **TC-4.1.1:** **Create a Percentage-Based Quantity Campaign:**
    1.  Go to "Add Campaign".
...
- [ ] **TC-4.1.2:** **Create a Fixed-Amount Quantity Campaign:**
...
- [ ] **TC-4.2.1:** **Shop Page Display:** View the shop page.
...
- [ ] **TC-4.2.2:** **Single Product Page Display:** View the product page for the "T-Shirt".
...
- [ ] **TC-5.1.1:** **Create an Early Bird Campaign:**
    1.  Go to "Add Campaign".
...
- [ ] **TC-5.2.1:** **First Order (First Tier):** With the campaign usage at 0, add any product to the cart and proceed to checkout.
    - **Expected:** A **50% discount** is applied to the product in the cart.
- [ ] **TC-5.2.2:** **Complete First Order:** Complete the purchase.
...
- [ ] **TC-5.2.3:** **Second Order (Still First Tier):** As a different customer (or in an incognito window), add a product to the cart.
    - **Expected:** A **50% discount** is still applied, as the usage count (1) is less than the tier max (2).
- [ ] **TC-5.2.4:** **Complete Second Order:** Complete the purchase.
    - **Expected:** Order is successful. The campaign usage count is now **2**.
- [ ] **TC-5.2.5:** **Third Order (Second Tier):** As a new customer, add a product to the cart.
...
- [ ] **TC-5.2.6:** **Complete Orders 3, 4, and 5.**
    - **Expected:** All three orders receive a 25% discount. After the 5th total order, the campaign usage count is **5**.
- [ ] **TC-5.2.7:** **Sixth Order (Offer Expired):** As a new customer, add a product to the cart.
